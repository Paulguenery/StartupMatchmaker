...
import { Shield, Star, LogOut, FileText, Image, Award } from "lucide-react";
      ...
      for (const key in data) {
        if (key === 'documents' && Array.isArray(data.documents) && data.documents.length > 0) {
          data.documents.forEach((doc: any, index: number) => {
            ...
            formData.append(`documents[${index}].url`, doc.url); // Assuming url is stored
          });
        } else {
              ...
              <form onSubmit={form.handleSubmit(data => updateProfileMutation.mutate(data))} className="space-y-4">
                ...
                {user?.role === "project_seeker" && (
                  <div className="space-y-4 border-t pt-4">
                    <h3 className="font-semibold">Documents professionnels</h3>
                    ...
                    <div className="mt-4">
                      ...
                      {form.getValues("documents")?.map((doc: any) => (
                        <div key={doc.type} className="flex items-center gap-2 text-sm text-gray-600">
